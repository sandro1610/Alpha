  ...
  $(document).ready(function () {
    ...
    function i() {
      ...
      $(".menu,.menu-button").addClass("menu-is-open"),
        $(".menu")
          .css({ transform: "translate(0," + -a + "px)" })
          .transition({ y: 0, duration: 400 });
      var b = $(".menu-link:not(.mobile-hide)");
      b.each(function (c) {
        $(this)
          .css({ transform: "translate(0," + -a + "px)" })
          .transition({ y: 0, duration: 400 + 70 * (b.length - c) });
      });
    }
    function j() {
      g = !1;
      ...
      $(".menu-button").removeClass("menu-is-open"),
        $(".menu").transition(
          { y: -a, duration: 250, easing: "easeInQuad" },
          function () {
    ...
    function p() {
      $(".preloader-container").transition(
        { scale: 0, rotate: 15, easing: "easeInQuad", duration: 400 },
        function () {
          $(".preloader-container").remove(),
            $(".cover").css({ transformOrigin: "0 0" }),
            $(".cover").transition(
              { y: "100%", skewY: 0, duration: 600, easing: "easeInQuad" },
              function () {
                $(".cover").remove();
              }
            ),
            $("#home .layer").each(function () {
              ...
              b.css({
                ...
                transform:
                  "scale(" + d + "," + d + ") translate(" + 30 * c + "px,0)",
              }),
                b.transition({ scale: 1, x: 0, duration: 1400 });
            });
        }
      );
    }
    function q() {
      ...
      var j = b * d * f,
        k = (h - j) / 2,
        l = 0;
      k > 0 && ((k = 0), (f = h / (b * d)), (l = (g - a * d * f) / 2)),
        $(".menu").attr("data-0", "transform:translate3d(0,0px,0)");
      var m = $(".menu").height(),
        n = h - m;
      $(".menu").attr("data-top", "transform:translate3d(0," + -n + "px,0)"),
        $(".parallax").each(function () {
          null != $(this).data("height") &&
            $(this).css({ height: $(this).data("height") * d * f });
        }),
        $(".parallax .layers").css({ left: l }),
        $(".garden .layers").css({ top: 0 }),
        $(".garden").css({ height: h });
      var o = 0;
      null != e && ((o = e.getScrollTop()), e.destroy(), (e = null)),
        g < mobileThreshold
          ? $(".menu").css({ lineHeight: h + "px" })
          : $(".menu").css({ lineHeight: "70px" });
      var p = $(".viewport").height() - h,
        q = $(".viewport").data("parallax-keyframes");
      null != q && $(".viewport").attr("data-" + q, null),
        $(".viewport").attr(
          "data-" + p,
          "transform:translate3d(0," + -p + "px,0)"
        ),
        $(".viewport").data("parallax-keyframes", p),
        $(".layer").each(function () {
          var a = $(this),
          ...
          if (
            ...
            var k = a.data("pos-y"),
              ...
              n = a.data("scale");
            if (
              (null != l ? a.css({ left: l * d * f }) : (l = 0),
              null != k ? a.css({ top: k * d * f }) : (k = 0),
              ...
              null == q && (q = 0);
              var s = 0.5;
              null != a.parent().data("parallax-strength") &&
                (s = a.parent().data("parallax-strength"));
              var t = 700,
                u = (t - t * m) * s;
              ...
              a.css({
                transformOrigin:
                  g / 2 - l * d * f + "px " + (0.9 * h - k * d * f) + "px",
              }),
                a.attr(
                  "data-" + keyStart,
                  "transform:translate3d(0," + -u + "px,0)" + v
                ),
                a.attr(
                  "data-" + keyEnd,
                  "transform:translate3d(0," + y + "px,0)" + w
                ),
                a.data("parallax-keyframes", [keyStart, keyEnd]);
            }
            r(a);
          }
        }),
        $(".garden .layers").css({ top: k }),
        $(".layer img").css({
          position: "absolute",
      ...
      a.find("canvas").remove(),
        (c.onload = function (e) {
          ...
          b.attr("width", h),
            ...
            b.css({ width: Math.round(h / g), height: Math.round(i / g) }),
            d.drawImage(c, 0, 0, h, i),
            n();
        });
    }
    function w() {
      $(".draw").each(function () {
        ...
        if (
          b != c &&
          (a.hasClass("line") && a.css({ transform: "scale(" + c + ",1)" }),
          a.data("loaded"))
        ) {
          var d = $(this).data("snap");
          if (null != d) {
            d.selectAll("path,line,polyline,polygon,ellipse").forEach(function (
              a
            ) {
              var b = a.attr("data-length");
              null == b && ((b = a.getTotalLength()), a.attr("data-length", b)),
                void 0 === b && (b = 1e3),
                a.attr({ strokeDasharray: c * b + "," + b });
            });
          }
          a.data("last-v", c);
        }
    ...
    var k = 0,
      l = $(".layer").length;
    touchCheck()
      ? (l -= $(".layer[data-no-touch]").length)
      : (l -= $(".layer[data-only-touch]").length),
      mobileCheck() && (l = 0);
    var m = !1;
    touchCheck() && $("video").remove(),
      (function () {
        ...
        a.each(function (a) {
          ...
          e.css({
            ...
            top: Math.sin(f) * c,
            animation:
              "ball-anim " + d + "s ease-in " + (d / b) * a + "s infinite",
          });
        }),
          $(".preloader").css({ visibility: "inherit" });
      })(),
      $(".portfolio-item").each(function () {
        var a = $(this),
          b = a.find(".inline-svg");
      ...
      $(".draw").each(function () {
        var a = $(this);
        a.attr("data-" + a.height() + "-bottom", "@data-v:0"),
          a.attr("data-80-center", "@data-v:1"),
          a.attr("data-v", 0);
      });
    touchCheck()
    ...
    $("a[href*=#]:not([href=#])").click(function () {
      if (
        ...
        if (
          ...
          return (
            $("html,body").animate({ scrollTop: a.offset().top }, 1500), !1
          );
      }
    });
  });
